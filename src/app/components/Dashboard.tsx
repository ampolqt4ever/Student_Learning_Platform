...
type ActiveView = 'student-home' | 'lessons' | 'students' | 'reports' | 'lesson-management' | 'teacher-dashboard' | 'admin-dashboard' | 'gesture-recognition' | 'analytics';

// Static gamification data for the student strip
...
export function Dashboard({ username, role, onLogout }: DashboardProps) {
  const [activeView, setActiveView] = useState<ActiveView>(role === 'admin' ? 'teacher-dashboard' : 'student-home');
  const [drawerOpen, setDrawerOpen] = useState(false);

  const menuItems = [
    { id: 'student-home' as const, label: 'Dashboard', icon: Home, roles: ['student'] },
    { id: 'teacher-dashboard' as const, label: 'Teacher Dashboard', icon: LayoutDashboard, roles: ['admin'] },
    { id: 'admin-dashboard' as const, label: 'Admin Dashboard', icon: Settings, roles: ['admin'] },
    { id: 'lessons' as const, label: 'Math Lessons', icon: BookOpen, roles: ['admin', 'student'] },
    { id: 'gesture-recognition' as const, label: 'Gesture Recognition', icon: Hand, roles: ['admin', 'student'] },
    { id: 'lesson-management' as const, label: 'Manage Lessons', icon: Upload, roles: ['admin'] },
    { id: 'students' as const, label: 'Student Management', icon: Users, roles: ['admin'] },
    { id: 'analytics' as const, label: 'Reports & Analytics', icon: BarChart3, roles: ['admin', 'student'] },
  ];

  const filteredMenuItems = menuItems.filter(item => item.roles.includes(role));

  const navigate = (id: ActiveView) => {
    setActiveView(id);
    setDrawerOpen(false);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      >
        ...
        {/* Student mini-progress inside drawer */}
        {role === 'student' && (
      ...
      {/* Gamification strip — students only */}
